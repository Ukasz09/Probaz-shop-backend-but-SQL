...
exports.create = async (req, res) => {
  ...
  Item.create(item)
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while creating the orderHistory.",
      });
    });
};
...
exports.dates = (req, res) => {
  ArchiveItem.findAll({
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while retrieving archiveItem. ",
      });
    });
};
...
exports.update = (req, res) => {
  ...
  orderHistory.findAll().then(function (item) {
    if (item) {
...
exports.findAll = async (req, res) => {
  ...
  orderHistory
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while retrieving archivedData. ",
      });
    });
};
